...
//end of merge code
	...
	cairo_font_options_set_subpixel_order(
		fo, to_cairo_subpixel_order(surface->subpixel));
	...
	if (state->args.show_indicator &&
		...
		case AUTH_STATE_BACKSPACE:
			...
			if (state->xkb.caps_lock &&
				...
				if (state->failed_attempts > 999) {
					...
					snprintf(attempts, sizeof(attempts),
						 "%d", state->failed_attempts);
					text = attempts;
				}
			}

			xkb_layout_index_t num_layout =
				xkb_keymap_num_layouts(state->xkb.keymap);
			if (!state->args.hide_keyboard_layout &&
			    (state->args.show_keyboard_layout ||
			     num_layout > 1)) {
				...
				while (curr_layout < num_layout &&
				       xkb_state_layout_index_is_active(
					       state->xkb.state, curr_layout,
					       XKB_STATE_LAYOUT_EFFECTIVE) !=
					       1) {
					++curr_layout;
				}
				// will handle invalid index if none are active
				layout_text = xkb_keymap_layout_get_name(
					state->xkb.keymap, curr_layout);
			}
			break;
		default:
			break;
		...
		if (layout_text) {
			...
			x = (buffer_width / 2) - (extents.width / 2) -
			    box_padding;
			y = buffer_diameter;
			...
			cairo_set_source_u32(
				cairo, state->args.colors.layout_background);
			cairo_fill_preserve(cairo);
			// border
			cairo_set_source_u32(cairo,
			...
			cairo_move_to(
				cairo, x - extents.x_bearing + box_padding,
...
void render_frame_touch_pin(struct swaylock_surface *surface) {
	...
	cairo_select_font_face(cairo, state->args.font, CAIRO_FONT_SLANT_NORMAL,
			       CAIRO_FONT_WEIGHT_NORMAL);
	if (state->args.font_size > 0 && state->args.font_size < 100) {
		cairo_set_font_size(cairo, state->args.font_size);
	} else {
