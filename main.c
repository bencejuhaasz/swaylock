...
static int parse_options(int argc, char **argv, struct swaylock_state *state,
	...
	while (1) {
		int opt_idx = 0;
		c = getopt_long(argc, argv, "c:deFfhi:s:tvC:", long_options,
				&opt_idx);
		if (c == -1) {
			break;
		}
...
static int load_config(char *path, struct swaylock_state *state) {
	...
	while ((nread = getline(&line, &line_size, config)) != -1) {
		...
		result = parse_options(2, argv, state, NULL);
		free(flag);
		if (result != 0) {
			break;
		}
...
int main(int argc, char **argv) {
	...
	int result = parse_options(argc, argv, NULL, &config_path);
	if (result != 0) {
		free(config_path);
		return result;
	}
	...
	if (argc > 1) {
		...
		int result = parse_options(argc, argv, &state, NULL);
		if (result != 0) {
			free(state.args.font);
			return result;
		}
